...
async fn handle_send(
    ...
    let uri_string = String::from_utf8(bytes.to_vec()).unwrap();
    log::info!("uri_string: {}", uri_string);
    let request: bip78::Uri<'_> =
        bip78::Uri::try_from(uri_string).map_err(|_| PayJoinError::Internal("Bad PayJoin uri"))?;
